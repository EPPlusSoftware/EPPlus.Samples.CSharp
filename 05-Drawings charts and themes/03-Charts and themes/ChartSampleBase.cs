...
namespace EPPlusSamples.DrawingsChartsAndThemes
{
    public abstract class ChartSampleBase
    {
        ...
        protected static async Task<ExcelRangeBase> LoadFromDatabase(ExcelWorksheet ws)
        {
        ...
        protected static async Task<ExcelRangeBase> LoadSalesFromDatabase(ExcelWorksheet ws)
        {
            ExcelRangeBase range;
            using (var sqlConn = new SQLiteConnection(SampleSettings.ConnectionString))
            {
                sqlConn.Open();
                var sql = SqlStatements.GroupedOrdersSql;
        ...
        protected static ExcelRangeBase CreateIceCreamData(ExcelWorksheet ws)
        {
