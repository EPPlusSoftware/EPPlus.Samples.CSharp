...
namespace EPPlusSamples.TablesPivotTablesAndSlicers
{
    ...
    public static class TablesSample
    {
        ...
        /// <returns></returns>
        private static async Task CreateTableWithACalculatedColumnAsync(ExcelPackage p)
        {
            var ws = p.Workbook.Worksheets.Add("SimpleTable");

            var range = await LoadDataAsync(ws).ConfigureAwait(false);
        ...
        /// <param name="p"></param>
        /// <returns></returns>
        private static async Task StyleTablesAsync(ExcelPackage p)
        {
            var ws = p.Workbook.Worksheets.Add("StyleTables");

            var range1 = await LoadDataAsync(ws).ConfigureAwait(false);
            ...
            var range2 = await LoadDataAsync(ws, "K1").ConfigureAwait(false);
        ...
        /// <param name="p">The package</param>
        /// <returns></returns>
        private static async Task CreateTableFilterAndSlicerAsync(ExcelPackage p)
        {
        ...
        private static async Task<ExcelRangeBase> LoadDataAsync(ExcelWorksheet ws, string startCell="A1")
        {
