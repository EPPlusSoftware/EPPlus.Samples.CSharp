...
namespace EPPlusSamples.LoadingData
{
    public static class LoadingDataWithDynamicObjects
    {
        public static void Run()
        {
            ...
            using(var package = new ExcelPackage(FileUtil.GetCleanFileInfo("2.1-LoadDynamicObjects.xlsx")))
            {
            ...
            var jsonItems = JsonConvert.DeserializeObject<IEnumerable<ExpandoObject>>(File.ReadAllText(FileUtil.GetFileInfo("02-Import and Export\\01-Load data from collection", "testdata.json").FullName));
            using (var package = new ExcelPackage(FileUtil.GetCleanFileInfo("2.1-LoadJsonFromFile.xlsx")))
            {
                var sheet = package.Workbook.Worksheets.Add("Dynamic");
